...
from traffic.common.sqlalchemyutils import paginate_query
from traffic import db
from traffic.db.sqlalchemy import models
from traffic.db.sqlalchemy.session import get_session
...
@require_context
def tqdisc_create(context, values, session=None):
    if not session:
        session = get_session()
    with session.begin():
        tqdisc_ref = models.Tqdisc()
        tqdisc_ref.update(values)
    
        tqdisc_ref.save(session=session)
    return tqdisc_ref['classid']     
...
def get_ip_by_instance(context, instanceid):
    ...
    float_ip = session.execute('select address from floating_ips where fixed_ip_id='+str(result[0]))
    return float_ip
...
def get_host_by_instance(context, instanceid):
    ...
    session = get_session()
    result = session.execute('select host from instances where uuid="'+instanceid+'"').first()
    return result
...
def tqdisc_get(context, id):
    ...
    if not result:
        raise exception.NoTqdisc(id=id)
...
@require_context
def tqdisc_get_by_classid(context, classid):
    result = model_query(context, models.Tqdisc, project_only=True).\
                 filter_by(classid=classid).\
                 first()
    if not result:
        raise exception.NoTqdisc(id=id)
...
def tqdisc_get_by_instance_idbk(context, instanceid):
    result = model_query(context, models.Tqdisc, project_only=False).\
                 ...
                 first()
    if not result:
        raise exception.NoTqdisc(instanceid=instanceid)
    return result
...
def tqdisc_delete(context, id):
    session = get_session()
    with session.begin():
        tqdisc_ref = tqdisc_get(context, id, session=session)
        tqdisc_ref.delete(session=session)
        
@require_context
def tqdisc_delete_by_classid(context, classid):
    session = get_session()
    with session.begin():
...
def tqdisc_get_host(context, classid):
    ...
    with session.begin():
        tqdisc_ref = tqdisc_get_by_classid(context, classid, session)
        return tqdisc_ref['host']
    
@require_context
def tfilter_get_by_classid(context, classid):
    result = model_query(context, models.Tfilter, project_only=True).\
                 filter_by(classid=classid).\
                 first()
    if not result:
...
def tfilter_get(context, id):
    result = model_query(context, models.Tfilter, project_only=True).\
                 ...
                 first()
    if not result:
        raise exception.NoTfilter(id=id)
    return result

@require_context
def tfilter_get_by_ip(context, ip):
    result = model_query(context, models.Tfilter, project_only=True).\
                 filter_by(ip=ip).\
                 first()
    if not result:
        raise exception.NoTfilter(ip=ip)
    return result

@require_context
def tfilter_get_by_flow_id(context, flow_id):
    result = model_query(context, models.Tfilter, project_only=True).\
...
@require_context
def tfilter_get_by_handle(context, handle):
    result = model_query(context, models.Tfilter, project_only=True).\
                 filter_by(handle=handle).\
                 first()
    if not result:
        raise exception.NoTfilter(handle=handle)
    return result

@require_context
def tfilter_delete(context, id):
    session = get_session()
    with session.begin():
        tfilter_ref = tfilter_get(context, id, session=session)
        tfilter_ref.delete(session=session)    

@require_context
def tfilter_delete_by_handle(context, handle):
    session = get_session()
    with session.begin():
...
@require_admin_context
def instance_get_all(context, columns_to_join=None, page_index=None, limit=None):
    ...
    if limit is not None:
        if page_index is not None:
            query = query.offset(int(page_index) * int(limit))
...
@require_context
def instance_count_all(context):
    session = get_session()
    result = {}
    if context.is_admin:
        ins_session = session.query(models.Instance).filter_by(deleted=0);
    else:
        ins_session = session.query(models.Instance).filter_by(deleted=0).filter_by(project_id=context.project_id)
	
    result['all_instances'] = ins_session.count()
    result['active_instances'] = ins_session.filter_by(vm_state='active').count()
    result['building_instances'] = ins_session.filter_by(vm_state='building').count()
    result['paused_instances'] = ins_session.filter_by(vm_state='paused').count()
    result['suspended_instances'] = ins_session.filter_by(vm_state='suspended').count()
    result['stopped_instances'] = ins_session.filter_by(vm_state='stopped').count()
    result['rescued_instances'] = ins_session.filter_by(vm_state='rescued').count()
    result['resized_instances'] = ins_session.filter_by(vm_state='resized').count()
    result['softdelete_instances'] = ins_session.filter_by(vm_state='soft-delete').count()
    result['deleted_instances'] = ins_session.filter_by(vm_state='deleted').count()
    result['error_instances'] = ins_session.filter_by(vm_state='error').count()
    return result


#add by weiyuanke@cnic.cn
@require_context
def instance_count_by_filters(context, **kwargs):
    session = get_session()
    ins_session = session.query(models.Instance)
    for k in kwargs:
        if kwargs[k] is not None:
            ins_session = ins_session.filter_by(k=kwargs[k])  
    return ins_session


@require_context
def instance_get_all_by_filters(context, filters, sort_key, sort_dir,
                                limit=None, marker=None, page_index=None):
    ...
    query_prefix = paginate_query(query_prefix, models.Instance, limit,
                           ...
                           marker=marker,
                           sort_dir=sort_dir, page_index=page_index)
...
#by weiyuanke@cnic.cn
@require_context
def instance_get_active_by_window(context, begin, end=None,
                                  project_id=None, host=None, limit=None, page_index=None, user_id=None):
...
def instance_get_active_by_window_joined(context, begin, end=None,
                                         project_id=None, host=None, limit=None, page_index=None):
...
@require_admin_context
def instance_get_all_by_host(context, host, limit=None, page_index=None):
    query = _instance_get_all_query(context).filter_by(host=host)
    if limit is not None:
        if page_index is not None:
            query = query.offset(int(page_index) * int(limit))
        query = query.limit(limit)
...
@require_admin_context
def instance_get_all_by_host_and_node(context, host, node, limit=None, page_index=None):
    query = _instance_get_all_query(context).filter_by(host=host).filter_by(node=node)
    if limit is not None:
        if page_index is not None:
            query = query.offset(int(page_index) * int(limit))
        query = query.limit(limit)
...
@require_admin_context
def instance_get_all_by_host_and_not_type(context, host, type_id=None, limit=None, page_index=None):
    query = _instance_get_all_query(context).filter_by(host=host).filter(models.Instance.instance_type_id != type_id)
    if limit is not None:
        if page_index is not None:
            query = query.offset(int(page_index) * int(limit))
        query = query.limit(limit)
...
@require_context
def instance_get_all_by_project(context, project_id, limit=None, page_index=None):
    authorize_project_context(context, project_id)
    query = _instance_get_all_query(context).filter_by(project_id=project_id)
    if limit is not None:
        if page_index is not None:
            query = query.offset(int(page_index) * int(limit))
        query = query.limit(limit)
...
@require_admin_context
def volume_get_all(context, limit=None, page_index=None):
...
#modified by weiyuanke@cnic.cn
@require_admin_context
def volume_get_all_by_host(context, host, limit=None, page_index=None):
    query = _volume_get_query(context).filter_by(host=host)
    if limit is not None:
        if page_index is not None:
            query = query.offset(int(page_index) * int(limit))
        query = query.limit(limit)
...
#modified by weiyuanke@cnic.cn
@require_admin_context
def snapshot_get_all(context, limit=None, page_index=None):
...
#modified by weiyuanke@cnic.cn
@require_context
def snapshot_get_all_by_project(context, project_id, limit=None, page_index=None):
    authorize_project_context(context, project_id)
    query = model_query(context, models.Snapshot).filter_by(project_id=project_id)
    if limit is not None:
        if page_index is not None:
            query = query.offset(int(page_index) * int(limit))
        query = query.limit(limit) 
...
@require_context
def update_instance_cdrom_active(context, instance_uuid, cdrom_active):
    session = get_session()
    with session.begin():
        session.query(models.Instance).\
                filter_by(uuid=instance_uuid).\
                update({'cdrom_active': cdrom_active})
...
def security_group_ensure_default(context, session=None):
    ...
    try:
        ...
        rule['cidr'] = '0.0.0.0/0'
        rule['from_port'] = -1
        security_group_rule_create(context, rule, session)
        rule = {}
        rule['parent_group_id'] = default_group.id
        rule['protocol'] = 'tcp'
        rule['to_port'] = 80
...
@require_context
def security_group_rule_create(context, values, session=None):
    if session is None:
        session = get_session()
