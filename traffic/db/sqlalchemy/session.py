...
import traffic.exception
import traffic.flags as flags
import traffic.openstack.common.log as logging
...
def get_session(autocommit=True, expire_on_commit=False):
    ...
    session = _MAKER()
    session.query = traffic.exception.wrap_db_error(session.query)
    session.flush = traffic.exception.wrap_db_error(session.flush)
    return session
    ...
    def _do_query(self, q):
        ...
        for file, line, method, function in traceback.extract_stack():
            ...
            if file.endswith('exception.py') and method == '_wrap':
                continue
            # traffic/db/api is just a wrapper around traffic/db/sqlalchemy/api
            if file.endswith('traffic/db/api.py'):
                continue
            # only trace inside traffic
            index = file.rfind('traffic')
            if index == -1:
                continue
