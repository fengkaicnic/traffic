...
from traffic.api.openstack.compute.views import servers as views_servers
...
class Controller(wsgi.Controller):
    ...
    def _get_servers(self, req, is_detail):
        ...
        if search_opts.get('vm_state') == 'active':
            search_opts['vm_state'] = 'active' 
        ...
        page_index = common.get_page_index(req)
        try:
    ...
    def _is_quantum_v2(self):
        return FLAGS.network_api_class ==\
            "traffic.network.quantumv2.api.API"
    ...
    def create(self, req, body):
        ...
        if '_is_precooked' in server['server'].keys():
            LOG.debug(_("_is_precooked ignore password."))
    ...
    def update(self, req, id, body):
        ...
        ctxt = req.environ['traffic.context']
        update_dict = {}
    ...
    def delete(self, req, id):
        """Destroys a server."""
        try:
            self._delete(req.environ['traffic.context'], req, id)
        except exception.NotFound:
            raise exc.HTTPNotFound()
        except exception.InstanceInvalidState as state_error:
            common.raise_http_conflict_for_instance_invalid_state(state_error,
    ...
    def _image_uuid_from_href(self, image_href):
        # If the image href was generated by traffic api, strip image_href
    ...
    def _get_server_admin_password(self, server):
        ...
        try:
            ...
            LOG.debug(_('Password in request %(password)s'), locals())
            self._validate_admin_password(password)
        except KeyError:
            LOG.debug(_('Invalid adminPass KeyError'))
            password = utils.generate_password(FLAGS.password_length)
        except ValueError:
            LOG.debug(_('Invalid adminPass ValueError'))
    ...
    def _validate_admin_password(self, password):
        if not isinstance(password, basestring):
            LOG.debug(_('Password Invalid %(password)s'), locals())
            raise ValueError()
