...
#add by wyk
block_migration = cfg.BoolOpt('block_migration',
        default=True,
        help='whether to use block_migration')

FLAGS = flags.FLAGS
FLAGS.register_opt(block_migration)
flags.DECLARE('consoleauth_topic', 'traffic.consoleauth')
...
def check_policy(context, action, target, scope='compute'):
    ...
    traffic.policy.enforce(context, _action, target)
...
class API(base.Base):
    ...
    def _check_num_instances_quota(self, context, instance_type, min_count,
        ...
        try:
            ...
            raise exception.TooManyInstances(overs=_(overs),
    ...
    def _create_instance(self, context, instance_type,
        ...
        LOG.debug(_("request_spec: %(request_spec)s, admin_password: %(admin_password)s , injected_files: %(injected_files)s , requested_networks: %(requested_networks)s , filter_properties: %(filter_properties)s "), locals())
    ...
    def get_active_by_window(self, context, begin, end=None, project_id=None, limit=None, page_index=None, user_id=None):
        """Get instances that were continuously active over a window."""
        return self.db.instance_get_active_by_window(context, begin, end,
                                                     project_id, limit=limit, page_index=page_index, user_id=user_id)
    ...
    def get_all(self, context, search_opts=None, sort_key='created_at',
                sort_dir='desc', limit=None, marker=None, page_index=None):
        ...
        inst_models = self._get_instances_by_filters(context, filters,
                                                     sort_key, sort_dir,
                                                     limit=limit,
                                                     marker=marker, page_index=page_index)
    ...
    def count_instances(self, context):
        return self.db.instance_count_all(context)

    #add by weiyuanke@cnic.cn
    def count_instances_by_filters(self, context, **kwargs):
        return self.db.instance_count_by_filters(context, **kwargs)


    def _get_instances_by_filters(self, context, filters,
                                  sort_key, sort_dir,
                                  limit=None,
                                  marker=None, page_index=None):
        ...
        return self.db.instance_get_all_by_filters(context, filters,
                                                   sort_key, sort_dir,
                                                   limit=limit, marker=marker, page_index=page_index)
    ...
    def backup(self, context, instance, name, backup_type, rotation,
        ...
        :param instance: traffic.db.sqlalchemy.models.Instance
    ...
    @wrap_check_policy
    @check_instance_lock
    def attach_image(self, context, instance, image_id, device=None):
        if (device == "" or device == None):
            device = "/dev/hdb"
        """Attach an existing image to an existing instance."""
        try:
            self.compute_rpcapi.attach_image(context, instance=instance,
                    image_id=image_id, mountpoint=device)
        except Exception:
            with excutils.save_and_reraise_exception():
    ...
    def detach_image(self, context, instance):
        try:
            self.compute_rpcapi.detach_image(context, instance=instance)
        except Exception:
            with excutils.save_and_reraise_exception():
                self.db.update_instance_cdrom_active(
                        context, instance['uuid'], True)
            # this code may not run, not sure about it. YANGYUAN
            return False
        return True
