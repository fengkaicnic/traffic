...
"""traffic base exception handling.

Includes decorator for re-raising traffic-type exceptions.
...
def wrap_exception(notifier=None, publisher_id=None, event_type=None,
    ...
    """
    # TODO(sandy): Find a way to import traffic.notifier.api so we don't have
    # to pass it in as a parameter. Otherwise we get a cyclic import of
    # traffic.notifier.api -> traffic.utils -> traffic.exception :(
...
class ImageNotFoundEC2(ImageNotFound):
    message = _("Image %(image_id)s could not be found. The traffic EC2 API "
...
class NoTqdisc(NotFound):
    message = _("Tqdisc %(tqdisc)s found.")
    
class NoTfilter(NotFound):
    message = _("Tfilter %(tfilter)s found")
...
class TooManyInstances(QuotaError):
    message = _("Quota exceeded for %(overs)s: Requested %(req)s, but already used %(used)d of %(allowed)d %(resource)s")
